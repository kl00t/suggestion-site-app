...
public partial class Details
{
    ...
    private async Task CompleteSetStatus()
    {
        switch (settingStatus)
        {
            ...
            case "watching":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"We noticed the interest in this suggestion. If more people are interested then we may address it in the future.";
                break;
            case "upcoming":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"Great suggestion. We have a resource in the pipeline to address the topic.";
                break;
            case "dismissed":
                suggestion.SuggestionStatus = statuses.Where(s => s.StatusName.ToLower() == settingStatus.ToLower()).First();
                suggestion.OwnerNotes = $"Sometimes a good idea doesn't fit within our scope and vision. This is one of them.";
                break;
            default:
                return;
        }
